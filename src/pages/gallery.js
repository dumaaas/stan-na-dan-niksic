...
export default function Gallery() {
  const numbers = new Array(36).fill(1).map((_, index) => index + 1);
        ...
        >
          <Masonry gutter="40px">
            {numbers.map((el, index) => (
              <div
                key={index}
                className="relative inline-block w-full h-full group z-[40] hover:cursor-pointer"
              >
                <div className="absolute w-full h-full overflow-hidden flex items-end justify-start p-[20px] text-white">
                  <div className="transition-all ease-in-out duration-200 right-0 z-[30] flex flex-col gap-[10px] w-full h-full top-0 absolute bg-[#1f3d4738] group-hover:backdrop-blur-[4px] group-hover:backdrop-blur-0 p-[30px] items-center justify-center "></div>
                  <div className="cursor-pointer flex items-center justify-center gap-[6px] ml-auto z-[99999] opacity-0 group-hover:opacity-100 transform translate-y-[10px] group-hover:translate-y-0 transition-all ease-in-out duration-500">
                    <p className="">Pogledaj sliku</p>
                    <svg
